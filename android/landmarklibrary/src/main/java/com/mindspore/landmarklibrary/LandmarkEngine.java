...
public final class LandmarkEngine {
    ...
    public void calculateExtraFacePoints(float[] vertexPoints, int index) {
        ...
        // 额头中心
        vertexPoints[FaceLandmark.headCenter * 2] = vertexPoints[FaceLandmark.eyeCenter * 2] * 2.0f
                - vertexPoints[FaceLandmark.noseLowerMiddle * 2];
        vertexPoints[FaceLandmark.headCenter * 2 + 1] = vertexPoints[FaceLandmark.eyeCenter * 2 + 1] * 2.0f
                - vertexPoints[FaceLandmark.noseLowerMiddle * 2 + 1];
    ...
    public synchronized void getEyeVertices(float[] vertexPoints, int faceIndex) {
        ...
        // 左眉毛下方中心点
        vertexPoints[38 * 2] = (mFaceArrays.get(faceIndex).vertexPoints[3 * 2] +
                mFaceArrays.get(faceIndex).vertexPoints[44 * 2]) * 0.5f;
        vertexPoints[38 * 2 + 1] = (mFaceArrays.get(faceIndex).vertexPoints[3 * 2 + 1] +
                mFaceArrays.get(faceIndex).vertexPoints[44 * 2 + 1]) * 0.5f;

        // 右眉毛下方中心点
        vertexPoints[39 * 2] = (mFaceArrays.get(faceIndex).vertexPoints[29 * 2] +
                mFaceArrays.get(faceIndex).vertexPoints[44 * 2]) * 0.5f;
        vertexPoints[39 * 2 + 1] = (mFaceArrays.get(faceIndex).vertexPoints[29 * 2 + 1] +
                mFaceArrays.get(faceIndex).vertexPoints[44 * 2 + 1]) * 0.5f;
    }
