...
public class CameraController implements ICameraController, Camera.PreviewCallback {
    ...
    private static Camera.Size calculatePerfectSize(List<Camera.Size> sizes, int expectWidth,
        ...
        sortList(sizes, expectWidth, expectHeight);
        // 根据计算类型判断怎么如何计算尺寸
        Camera.Size perfectSize = getPerfectSize(bigEnough, noBigEnough, expectWidth, expectHeight, calculateType);
    ...
    private static List<Camera.Size> bigEnough;
    private static List<Camera.Size> noBigEnough;

    private static void sortList(List<Camera.Size> sizes, int expectWidth,
                                 int expectHeight) {
        // 根据宽度进行排序
        Collections.sort(sizes, new CompareAreaSize());
        // 根据当前期望的宽高判定
        bigEnough = new ArrayList<>();
        noBigEnough = new ArrayList<>();
        for (Camera.Size size : sizes) {
    ...
    private static Camera.Size getPerfectSize(List<Camera.Size> bigEnough, List<Camera.Size> noBigEnough,
                                              int expectWidth, int expectHeight, CalculateType calculateType) {
