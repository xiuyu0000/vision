...
import com.mindspore.facelibrary.resource.ResourceCodec;
import com.mindspore.facelibrary.resource.ResourceIndexCodec;
import com.mindspore.facelibrary.stickers.bean.DynamicStickerData;
...
public class DynamicStickerLoader {
    ...
    public DynamicStickerLoader(boolean isStaticSticker, DynamicStickerBaseFilter filter,
                                DynamicStickerData stickerData, String folderPath) {
        ...
        if (pair != null) {
            mResourceIndexCodec = new ResourceIndexCodec(mFolderPath + "/" + (String) pair.first,
                    mFolderPath + "/" + (String) pair.second);
        }
...
//                mWeakFilter.get().restartPlayer();
        ...
        drawBitmapSticker(frameIndex);
    }

    private void drawBitmapSticker(int frameIndex) {
        // 根据帧索引读取贴纸
        Bitmap bitmap = null;
        if (mResourceIndexCodec != null) {
