...
public class BoundingBox implements Serializable {
    ...
    public BoundingBox ext(BoundingBox bounds) {
        return this.set(min.set(min(min.x, bounds.min.x), min(min.y, bounds.min.y), min(min.z, bounds.min.z)),
                max.set(max(max.x, bounds.max.x), max(max.y, bounds.max.y), max(max.z, bounds.max.z)));
    }
    ...
    public BoundingBox ext(Vector3 center, float radius) {
        return this.set(min.set(min(min.x, center.x - radius),
                min(min.y, center.y - radius),
                min(min.z, center.z - radius)),
                max.set(max(max.x, center.x + radius),
    ...
    public boolean contains(BoundingBox b) {
        return !isValid()
                || (min.x <= b.min.x && min.y <= b.min.y && min.z
                <= b.min.z && max.x >= b.max.x && max.y >= b.max.y && max.z >= b.max.z);
    }
