...
import com.mindspore.enginelibrary.utils.FaceppConstraints;
...
public final class FaceTracker {
    ...
    public static void requestFaceNetwork(Context context) {
        ...
        String url = FaceppConstraints.CN_LICENSE_URL;
    ...
    private static class TrackerThread extends Thread {
        ...
        private synchronized void internalTrackFace(byte[] data, int width, int height) {
            ...
            // 计算人脸关键点
            checkFacePoint(facepp, faces, orientation, width, height);
            // 设置人脸个数
    ...
    private static void checkFacePoint(Facepp facepp, Facepp.Face[] faces, int orientation, int width, int height) {
        FaceTrackParam faceTrackParam = FaceTrackParam.getInstance();
        // 计算人脸关键点
        if (faces != null && faces.length > 0) {
            for (int index = 0; index < faces.length; index++) {
                ...
                checkOneFacePoint(index,orientation,face,width,height);
                // 插入人脸对象
    ...
    public static void checkOneFacePoint(int index,int orientation,Facepp.Face face,int width, int height){
        FaceTrackParam faceTrackParam = FaceTrackParam.getInstance();
        OneFace oneFace = LandmarkEngine.getInstance().getOneFace(index);
