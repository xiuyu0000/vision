 ...
 */
Detection::Detection(int srcImgWidth, int srcImgHeight) {
...
std::string Detection::getDecodeResult(float *branchScores, float *branchBoxData) {
    std::string result = "";
    NormalBox tmpBox[1917] = {0};
    float mScores[1917][sizeof(g_thres_map) / sizeof(int)] = {0};
    ...
    for (int i = 0; i < 1917; ++i) {
        for (int j = 0; j < sizeof(g_thres_map) / sizeof(int); ++j) {
            mScores[i][j] = branchScores[i * sizeof(g_thres_map) / sizeof(int) + j];
        }
    }
    ...
    for (int i = 1; i < sizeof(g_thres_map) / sizeof(int); i++) {
...
void Detection::getDefaultBoxes() {
...
void Detection::ssd_boxes_decode(const NormalBox *boxes,
...
void Detection::nonMaximumSuppression(const YXBoxes *const decoded_boxes,
...
double Detection::IOU(float r1[4], float r2[4]) {
