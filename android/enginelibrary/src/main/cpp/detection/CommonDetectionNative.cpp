...
bool CommonDetectionBitmapToLiteMat(JNIEnv *env, const jobject &srcBitmap, LiteMat *lite_mat) {
    bool ret;
...
bool CommonDetectionPreProcessImageData(const LiteMat &lite_mat_bgr, LiteMat *lite_norm_mat_ptr) {
    bool ret;
...
char *CommonDetectionCreateLocalModelBuffer(JNIEnv *env, jobject modelBuffer) {
...
ProcessRunnetResult(std::unordered_map<std::string, mindspore::tensor::MSTensor *> msOutputs,
    ...
    Detection Detection(srcImageWidth, srcImageHeight);

    std::string retStr = Detection.getDecodeResult(tmpscores2, tmpdata);
...
Java_com_mindspore_enginelibrary_train_DetectionTrain_loadModel(JNIEnv *env, jobject thiz,
                                                         jobject buffer,
                                                         jint numThread) {
    if (nullptr == buffer) {
    ...
    char *modelBuffer = CommonDetectionCreateLocalModelBuffer(env, buffer);
...
Java_com_mindspore_enginelibrary_train_DetectionTrain_runNet(JNIEnv *env, jobject thiz,
                                                      jlong netEnv,
                                                      jobject srcBitmap) {
    LiteMat lite_mat_bgr, lite_norm_mat_cut;

    if (!CommonDetectionBitmapToLiteMat(env, srcBitmap, &lite_mat_bgr)) {
        MS_PRINT("CommonDetectionBitmapToLiteMat error.");
        return NULL;
    }
    int srcImageWidth = lite_mat_bgr.width_;
    int srcImageHeight = lite_mat_bgr.height_;
    if (!CommonDetectionPreProcessImageData(lite_mat_bgr, &lite_norm_mat_cut)) {
        MS_PRINT("CommonDetectionPreProcessImageData error.");
        return NULL;
    }
...
Java_com_mindspore_enginelibrary_train_DetectionTrain_unloadModel(JNIEnv *env,
                                                           jobject thiz,
                                                           jlong netEnv) {
    void **labelEnv = reinterpret_cast<void **>(netEnv);
    MSNetWork *labelNet = static_cast<MSNetWork *>(*labelEnv);
