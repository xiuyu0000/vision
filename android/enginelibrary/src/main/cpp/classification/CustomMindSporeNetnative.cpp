...
#include "CustomMindSporeNetnative.h"
#include "MSNetWork.h"
 ...
 * @return
 */
std::string ProcessRunnetResult(int RET_CATEGORY_SUM,
    ...
    for (int i = 0; i < RET_CATEGORY_SUM; ++i) {
        sumExp += exp(temp_scores[i]);
    }
    for (int i = 0; i < RET_CATEGORY_SUM; ++i) {
        if (temp_scores[i] > max) {
            max = exp(temp_scores[i]) / sumExp;
            maxIndex = i;
        }
    }
    ...
    categoryScore += ":";
    categoryScore += std::to_string(max);
...
Java_com_mindspore_enginelibrary_train_ClassificationTrain_loadCustomModel(JNIEnv *env,
                                                                    jobject thiz,
...
Java_com_mindspore_enginelibrary_train_ClassificationTrain_runCustomNet(JNIEnv *env, jobject type,
                                                                 jlong netEnv,
                                                                 jobject srcBitmap,
                                                                 jint ret_detailed_sum) {
    ...
    std::string resultStr = ProcessRunnetResult(ret_detailed_sum, msOutputs);
...
Java_com_mindspore_enginelibrary_train_ClassificationTrain_unloadCustomModel(JNIEnv *env,
                                                                      jobject type,
                                                                      jlong netEnv) {
