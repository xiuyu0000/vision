...
def squeezenet_infer(args_opt):
    ...
    for data in dataset_infer.create_dict_iterator(output_numpy=True):
        ...
        for i, v in enumerate(label):
            ...
            output = {v: predict}
            print(output)
            show_result(img=image_list[i],
                        result=output, out_file=image_list[i])


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='SqueezeNet infer.')
    ...
    parser.add_argument('--resize', type=int, default=224,
                        help='Resize the height and weight of picture.')

    args = parser.parse_known_args()[0]
    squeezenet_infer(args)
