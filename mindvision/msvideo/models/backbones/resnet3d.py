...
class ResidualBlockBase3D(nn.Cell):
    ...
    def __init__(self,
        ...
        if not norm:
            norm = nn.BatchNorm3d
        assert group != 1 or base_width == 64, "ResidualBlockBase3D only supports groups=1 and base_width=64"
...
class ResNet3D(nn.Cell):
    ...
    Args:
        ...
        stage_channels (list[int]): Output channel for every res stage. Default: [64, 128, 256, 512].
        stage_strides (list[list[int]]): Strides for every res stage.
            Default:[[1, 1, 1],
                     [1, 2, 2],
                     [1, 2, 2],
                     [1, 2, 2]].
