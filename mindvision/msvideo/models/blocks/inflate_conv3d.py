...
class Inflate3D(nn.Cell):
    ...
    Args:
        ...
        conv2_group (int): Splits filter into groups for the second conv layer, in_channels and out_channels
        ...
        activation (List[Optional[Union[nn.Cell, str]]]): Activation function which will be stacked on top of the
            normalization layer (if not None), otherwise on top of the conv layer. Default: nn.ReLU, None.
        inflate (bool): Whether to inflate two conv3d layers. If `True`, kernel size of two conv layers will be
            `(kernel_size[0], 1, 1)` and `(1, kernel_size[1], kernel_size[2])`. If `False`, kernel size of two
            conv layers will be `(1, 1, 1)` and `(kernel_size[0], kernel_size[1], kernel_size[2])`.
