...
class GridAnchorGenerator:
    ...
    def __init__(self, image_shape, scale, scales_per_octave, aspect_ratios, steps):
        ...
        self.steps = steps
        self.default_boxes, self.default_boxes_tlbr = self.generate_multi_levels()
    ...
    def generate(self, step):
        ...
        bbox_corners = bbox_corners / np.array([*self.image_shape, *self.image_shape]).astype(np.float32)
        bbox_centers = np.concatenate([bbox_centers, bbox_sizes], axis=1)
        bbox_centers = bbox_centers / np.array([*self.image_shape, *self.image_shape]).astype(np.float32)

        print(self.bbox_centers.shape)
        return bbox_centers, bbox_corners

    def generate_multi_levels(self):
        """For generating_multi_levels"""
        bbox_centers_list = []
        bbox_corners_list = []
        ...
        bbox_centers = np.concatenate(bbox_centers_list, axis=0)
        bbox_corners = np.concatenate(bbox_corners_list, axis=0)
        return bbox_centers, bbox_corners
